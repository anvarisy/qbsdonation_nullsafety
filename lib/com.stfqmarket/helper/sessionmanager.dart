...
class SessionManager {
  ...
  static Future<SessionData?> get sessionData async {
    try {
      final pref = await SharedPreferences.getInstance();
      return SessionData(
          pref.get(USER_EMAIL).toString(), pref.get(USER_FULLNAME).toString(), pref.get(USER_PHONE).toString(),
          pref.get(USER_REFERRAL).toString(), pref.get(USER_LEVEL) as int, pref.get(EXPIRES_IN).toString(), pref.get(TOKEN).toString());
    } catch (e) {
      print(e);
    }
    return null;
