...
class CartViewViewModel extends ChangeNotifier {
  ...
  Future<List<Cart>> _loadCheckoutCarts(user_profil profil) async {
    ...
    List<String> _savedTenantData = [];
    List<String> _savedProductData = [];
    ...
    if (maxProgress == 0) return [];
      ...
      for (final tenantId in _savedTenantData) {
        ...
        var item = await FirebaseFirestore.instance.collection('stfq-market').doc('Tenants').collection('items').doc(tenantId).get();
      ...
      for (final product in _savedProductData) {
        ...
        var item = await FirebaseFirestore.instance.collection('stfq-market').doc('Products').collection('items').doc(productId).get();
        var docProduct = Product.toProduct(item);
        docProduct.count = productCount;
        products[item.id] = docProduct;
      }
  ...
  Future<List<Cart>> _loadPaymentCarts(String userId) async {
    var documents = await FirebaseFirestore.instance.collection('stfq-market').doc('Carts').collection('items')
        .where('user.uid', isEqualTo: userId).where('status', isEqualTo: Constant.orderStatus.pendingPayment).get();

    List<Cart> carts = Cart.toList(documents.docs);
    return carts;
  }

  Future<List<Cart>> _loadPostPaymentCarts(String userId) async {
    var documents = await FirebaseFirestore.instance.collection('stfq-market').doc('Carts').collection('items')
        .where('user.uid', isEqualTo: userId).get();
